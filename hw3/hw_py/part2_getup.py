...
class MyGetupEnv(Getup):
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        body_ang_vel = self.get_gyro(data) # (3,)
        ...
        # others
        # flag: reach and decrease joint_qvel to zero
        # use clip to control reward larger than zero 

        # 计算各奖励项
        height_diff = DESIRED_BODY_HEIGHT - body_pos[2]
        flag = height_diff <= 0.05  # 高度接近目标的标志

        rew_height = jp.exp(-3 * height_diff**2)  # 高度奖励
        rew_orientation = -5.0 * jp.sum(jp.square(self.get_upvector(data)[:2]))  # 姿态惩罚
        joint_stability = jp.exp(-jp.linalg.norm(joint_qpos - default_qpos))  # 关节稳定性
        ang_vel_stability = jp.exp(-jp.linalg.norm(body_ang_vel))  # 角速度稳定性

        base_reward = 0.2 * rew_height + rew_orientation + joint_stability
        bonus_reward = flag * (ang_vel_stability)  # 只有高度达标后才计算

        reward = base_reward + bonus_reward
        # reward=jp.clip(reward,0.,150.)
...
def train_ppo():
    ...
    # 需要在 PPO 训练中确保 env_steps 是正确的整数类型
