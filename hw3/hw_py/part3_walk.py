...
class MyWalkEnv(Joystick):
    def step(self, state: mjx_env.State, action: jax.Array):
        ...
        '''
        I need to know if the 'body_lin_vel' is a scaler.
        Yaw axis is the 'z axis'.
        When implementing the YAW_ANG_VEL: "body_ang_vel" is in the body frame, but "DESIRED_YAW_ANG_VEL" 
        is in the world frame. They are different because gravity_vector(in local frame is not always 
        parallel with the up_vec.)
        Solutions: Get the rotation matrix, then project Vx, Vy to the world frame.
        '''
        def rotation_matrix_between_vectors(a, b):
            a = a + 1e-6 * (jp.linalg.norm(a) < 1e-6) 
            b = b + 1e-6 * (jp.linalg.norm(b) < 1e-6)
            
            a_norm = a / jp.linalg.norm(a)
            b_norm = b / jp.linalg.norm(b)
            
            v = jp.cross(a_norm, b_norm)
            c = jp.dot(a_norm, b_norm)
            denominator = 1 / (1 + jp.clip(c, -0.9999, 0.9999)) 
            
            vx = jp.array([[0, -v[2], v[1]],
                        [v[2], 0, -v[0]],
                        [-v[1], v[0], 0]])
            return jp.eye(3) + vx + jp.dot(vx, vx) * denominator

        
        rotation_matrix = rotation_matrix_between_vectors(gravity_vector, up_vec)
        # DESIRED_XY_LIN_VEL
        
        world_lin_vel_xy = rotation_matrix[:2, :2] @ body_lin_vel[:2]
        vel_direction_reward = jp.dot(
            DESIRED_XY_LIN_VEL / (jp.linalg.norm(DESIRED_XY_LIN_VEL) + 1e-6),
            world_lin_vel_xy / (jp.linalg.norm(world_lin_vel_xy) + 1e-6)
        )

        tracking_lin_vel = (
            0.7 * jp.exp(-0.5 * jp.linalg.norm(DESIRED_XY_LIN_VEL - world_lin_vel_xy)) +
            0.3 * vel_direction_reward
        )
        # tracking_lin_vel = jp.linalg.norm(DESIRED_XY_LIN_VEL - world_lin_vel_xy)
                
        # DESIRED_YAW_ANG_VEL
        world_ang_vel = rotation_matrix @ body_ang_vel
        tracking_ang_vel = jp.exp(-jp.linalg.norm(DESIRED_YAW_ANG_VEL - world_ang_vel[2]))
        ...
        rew_pose =  self._reward_pose(data.qpos[7:]) # Stay close to the default pose.
