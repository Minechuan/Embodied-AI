...
def execute_plan(env, gra_plan: np.ndarray,obj_pose,plan_type:int) -> bool:
    ...
    if plan_type == 1: 
        ...
        #     )
            ...
            #     initial_ok = True
            #     break
        gra_pre = [gra_plan[0][:7] for _ in range(3)]
        env.step_env(humanoid_action=gra_pre)
        ...
        #         )
        gra_0 = [gra for gra in gra_plan for _ in range(3)]
        env.step_env(humanoid_action=gra_0)
        ...
        #         )
        gra_1 = [gra for gra in gra_plan for _ in range(3)]
        env.step_env(humanoid_action=gra_1)
        ...
        #         )
        #     qpos = env.get_state()
        list_gra = [gra for gra in gra_plan for _ in range(8)]
        env.step_env(humanoid_action=list_gra)
        ...
        #         )
        env.step_env(humanoid_action=gra_plan)
        ...
        #             humanoid_action=gra_plan, 
        #         )
        gra_4 = [gra for gra in gra_plan for _ in range(5)]
        env.step_env(humanoid_action=gra_4)
...
def main():
    ...
    if not DISABLE_MOVE:
        ...
        while True:
            ...
            if con_w is None:
                ...
                if head_qp[1]>0.36:
                    break
                print("*"*80,"\nNot Tag detected\n","*"*80)
                head_qp[1] = max(0.1+head_qp[1],0.366)
                env.step_env(
                    humanoid_head_qpos=head_qp,
                )
                continue

            else:
                con_w = T_to_pose7d(con_w)
                con_xyz, con_quat = con_w[:3],con_w[3:]
                print(con_w)
                if con_xyz[0] < 1.3:
                    break
                else:
                    all_quad_command.append(quad_command_1)
                    env.step_env(
                            quad_command=quad_command_1
                        )
        ...
        quad_command_2 = [0,0,0.8]    
        # all_quad_command.append(quad_command_2)
        # for i in range(30):
        all_quad_command.append(quad_command_2)
        env.step_env(
                quad_command=quad_command_2
            )

        print("I think it is near the container, now I will turn around.")
        # for i in range(30):
        all_quad_command.append(quad_command_2)
        env.step_env(
                quad_command=quad_command_2
        ...
        quad_command_3 = [0.03,-0.4,0]
            # all_quad_command.append(quad_command_3)
        # for i in range(10):
        all_quad_command.append(quad_command_3)
        env.step_env(
                    quad_command=quad_command_3
        )
        from time import sleep
        sleep(20)


        num_come_steps = len(all_quad_command)
        all_quad_command.reverse()
        ...
        for i in range(num_come_steps):
            quad_command = all_quad_command[i]
            quad_command = [ -x for x in all_quad_command[i] ] 
