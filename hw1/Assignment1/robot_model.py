...
class RobotModel:
    ...
    def fk(self, qpos: np.ndarray) -> np.ndarray:
        ...
        """
        # task: use joint angle to compute line coordination

        joint_num = len(self.joints)
        link_num = joint_num + 1  
        ans = np.zeros((link_num, 4, 4))  # Initialize the answer array with shape (L, 4, 4)
        ans[0]=np.eye(4)
        theta_idx=0
        for i in range(joint_num):
            joint = self.joints[i]
            T = np.eye(4)
            # find the joint's origion frame
            T[:3, :3] = joint.rot  # Rotation part
            T[:3, 3] = joint.trans  # Translation part
            R=np.eye(3)
            # the first joint is fixed joint
            if isinstance(joint, RevoluteJoint):
                # attention! the link i consider the its parent's(joint i-1) rotation
                theta = qpos[theta_idx]  # Joint angle
                theta_idx+=1
                theta = np.clip(theta, joint.lower_limit, joint.upper_limit)
                axis = joint.axis / np.linalg.norm(joint.axis)  # Normalize axis
                aa=axis*theta
                R=axis_angle_to_mat(aa)
                # Apply rotation
                T[:3, :3] = T[:3, :3]@R
            ans[i+1]=np.dot(ans[i],T)
        # my bug is from misunder the number of fixed and revolute joint 
        # and use the wrong angle
        return ans
...
if __name__ == "__main__":
    ...
    #print(len(robot_model.joints),len(robot_model.links))
